...
(function () {
  ...
  function liveServerUrl(path, params = {}) {
  ...
  async function fetchPendingCount() {
    try {
      const res = await fetch(
        liveServerUrl('/manual-edit-stash', { pageUrl: location.pathname }),
      );
      if (!res.ok) return;
  ...
  async function onPendingPillClick() {
    ...
    try {
      const res = await fetch(
        liveServerUrl('/manual-edit-commit', { pageUrl: location.pathname, async: '1' }),
  ...
  async function onPendingTrashClick() {
    ...
    try {
      const res = await fetch(
        liveServerUrl('/manual-edit-discard', { pageUrl: location.pathname }),
        { method: 'POST' },
      );
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      const res = await fetch(
        liveServerUrl('/manual-edit-commit', { pageUrl: location.pathname, async: '1', repair: '1' }),
        { method: 'POST', keepalive: true },
      );
      if (!res.ok) throw new Error('HTTP ' + res.status);
      if (pendingKeepFixingBtn) pendingKeepFixingBtn.style.display = 'none';
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      const res = await fetch(
        liveServerUrl('/manual-edit-repair-decision', { pageUrl: location.pathname }),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function loadSvelteComponentParams(manifest) {
    ...
    const url = liveServerUrl('/source', { path: paramsPath });
    try {
  ...
  async function loadSvelteComponentVariantSource(manifest, variantNum) {
    ...
    const url = liveServerUrl('/source', { path: sourcePath });
    try {
      const res = await fetch(url);
      if (!res.ok) return '';
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    const url = liveServerUrl('/source', { path: manifestPath });
    try {
      const res = await fetch(url);
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    const url = liveServerUrl('/source', { path: filePath });
    fetch(url)
  ...
  function connectSSE() {
    evtSource = new EventSource(liveServerUrl('/events'));
    ...
    if (blob && hasAnnotations) {
      try {
        const uploadRes = await fetch(
          liveServerUrl('/annotation', { eventId: basePayload.id }),
...
void main() {
  ...
  function fetchAgentPollingStatus() {
    fetch(liveServerUrl('/status'), { cache: 'no-store' })
  ...
  async function fetchDesignSystem() {
    ...
    try {
      const [jsonRes, rawRes] = await Promise.all([
        fetch(liveServerUrl('/design-system.json'), { cache: 'no-store' }),
        fetch(liveServerUrl('/design-system/raw'), { cache: 'no-store' }),
      ]);
